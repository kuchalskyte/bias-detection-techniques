...
stratified_bootstrap <- function(data, variable1 = "Colour", 
  ...
  # Initialise an empty list to store the bootstrap samples
  bootstrap_samples <- list()

  # Perform stratified bootstrapping
  for (i in 1:n_resamples) {
    
    # Initialise an empty data frame for each bootstrap sample
...
auc_anova_1 <- function(data, variable1 = "Colour", 
  ...
  for (i in 1:n_resamples) {
    ...
    auc_results[(i-1)*2 + 1, ] <- c(i, "Blue", as.numeric(auc_blue))
    auc_results[(i-1)*2 + 2, ] <- c(i, "Red", as.numeric(auc_red))
  }
  ...
  plot <- ggplot(auc_results, aes(x = AUC, fill = Group)) +
    ...
    labs(title = "Density Plot of AUC Distributions by Group (Blue vs. Red)",
         x = "AUC",
         y = "Density",
...
auc1_analysis_heat <- function(societies, n_resamples = 100, seed, dist = 0.2, 
  ...
  heatmap <- ggplot(heatmap_data, aes(x = b2, y = b3, fill = Normalized_Fvalue)) +
    geom_tile() +
    scale_fill_viridis(option = "magma", name = "Normalized F-value") +
  ...
  return(list(
    heatmap = heatmap,                     # Heatmap for normalised F-values
    normalized_df = results_df,            # Normalised F-values
